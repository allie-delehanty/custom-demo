...
export const Default = ({
  ...
  return (
    <div
      className={cn("component feature-cards-grid", styles)}
...
export const Carousel = ({
  ...
  useEffect(() => {
    const update = () => {
      ...
      setVisibleCount(
        w < 640 ? VISIBLE.sm : w < 1024 ? VISIBLE.md : VISIBLE.lg,
      );
    };
    update();
                    ...
                    <div className="group relative flex flex-col overflow-hidden rounded-[var(--brand-card-radius,0.75rem)] h-full">
                      ...
                      {!card.cardImage?.jsonValue?.value?.src && !isEditing && (
                        <div className="flex flex-1 flex-col p-6">
                          ...
                          {(card.cardDescription?.jsonValue?.value ||
                            isEditing) && (
                            <ContentSdkRichText
                              field={card.cardDescription?.jsonValue}
...
const orderCards = (
  cards: FeatureCardItemFields[],
  titles: string[],
): FeatureCardItemFields[] =>
  [...cards].sort((left, right) => {
    const leftIndex = titles.indexOf(left.cardTitle?.jsonValue?.value ?? "");
    const rightIndex = titles.indexOf(right.cardTitle?.jsonValue?.value ?? "");
    return (
      (leftIndex < 0 ? titles.length : leftIndex) -
      (rightIndex < 0 ? titles.length : rightIndex)
    );
  });

const PRODUCT_ORDER = [
  "Low-Resistance Current Sensor",
  "24V Hall-effect Switches",
...
const ArrowLink = ({
  ...
  <ContentSdkLink
    field={field}
    aria-label={label}
    className="inline-flex h-8 w-8 items-center justify-center text-xl leading-none transition-transform hover:translate-x-1"
...
/* AllegroFeaturedProducts variant — flat three-up product cards on muted gray */
export const AllegroFeaturedProducts = ({
  ...
  const cards = orderCards(datasource.children?.results ?? [], PRODUCT_ORDER);
  ...
  return (
    ...
    >
      <section
        className="w-full px-6 py-14 md:py-16"
        style={{ backgroundColor: "var(--brand-muted)" }}
      >
        <div className="mx-auto max-w-7xl">
          <div className="mb-9 max-w-[610px] text-left">
            {(datasource.title?.jsonValue?.value || isEditing) && (
              <Text
                field={datasource.title?.jsonValue}
                tag="h2"
                className="text-3xl font-semibold tracking-tight md:text-[36px] font-[var(--brand-heading-font,inherit)]"
            ...
            {(datasource.description?.jsonValue?.value || isEditing) && (
              <ContentSdkRichText
                field={datasource.description?.jsonValue}
                className="mt-4 text-[15px] leading-6 font-[var(--brand-body-font,inherit)]"
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {cards.map((card) => (
              <article key={card.id} className="flex min-w-0 flex-col">
                {(card.cardImage?.jsonValue?.value?.src || isEditing) && (
                  <div
                    className="flex aspect-[1.55/1] items-center justify-center overflow-hidden"
                ...
                <div className="flex flex-1 flex-col pt-4">
                  {(card.cardTitle?.jsonValue?.value || isEditing) && (
                    <Text
                      field={card.cardTitle?.jsonValue}
                      tag="h3"
                      className="text-[19px] font-medium leading-6 font-[var(--brand-heading-font,inherit)]"
                      style={{ color: "var(--brand-fg)" }}
                    />
                  )}
                  {(card.cardDescription?.jsonValue?.value || isEditing) && (
                    <ContentSdkRichText
                      field={card.cardDescription?.jsonValue}
                      className="mt-2 flex-1 text-xs leading-[1.55] font-[var(--brand-body-font,inherit)]"
                      style={{ color: "var(--brand-muted-foreground)" }}
                    />
                  )}
                  {(card.cardLink?.jsonValue?.value?.href || isEditing) && (
                    <div className="mt-2 flex justify-end">
                      <ArrowLink
                        field={card.cardLink?.jsonValue}
                        label={`View ${card.cardTitle?.jsonValue?.value || "product"}`}
                      />
                    </div>
                  )}
                </div>
...
/* AllegroStoryMosaic variant — alternating split text and image tiles */
export const AllegroStoryMosaic = ({
  ...
  const cards = orderCards(datasource.children?.results ?? [], STORY_ORDER);
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-3 md:grid-cols-2">
            {cards.map((card, index) => {
              const imageFirstOnDesktop = index >= 2;
              return (
                <article
                  key={card.id}
                  className="grid min-h-[250px] overflow-hidden border md:grid-cols-2"
                  style={{ borderColor: "var(--brand-border)" }}
                >
                  {(card.cardImage?.jsonValue?.value?.src || isEditing) && (
                    <div
                      className={cn(
                        "order-1 min-h-[220px] overflow-hidden md:min-h-0",
                  ...
                  <div
                    className={cn(
                      "order-2 flex min-h-[220px] flex-col p-6 md:min-h-0 md:p-7",
                  ...
                  >
                    {(card.cardTitle?.jsonValue?.value || isEditing) && (
                      <Text
                        field={card.cardTitle?.jsonValue}
                        tag="h3"
                        className="text-lg font-medium font-[var(--brand-heading-font,inherit)]"
                        style={{ color: "var(--brand-fg)" }}
                      />
                    )}
                    {(card.cardDescription?.jsonValue?.value || isEditing) && (
                      <ContentSdkRichText
                        field={card.cardDescription?.jsonValue}
                        className="mt-3 text-xs leading-[1.6] font-[var(--brand-body-font,inherit)]"
                        style={{ color: "var(--brand-muted-foreground)" }}
                      />
                    )}
                    {(card.cardLink?.jsonValue?.value?.href || isEditing) && (
                      <div className="mt-auto flex justify-end pt-5">
                        <ArrowLink
                          field={card.cardLink?.jsonValue}
                          label={`View ${card.cardTitle?.jsonValue?.value || "story"}`}
                        />
                      </div>
                    )}
                  </div>
